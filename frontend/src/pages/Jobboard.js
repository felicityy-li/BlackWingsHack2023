...
import { Typography, Container, CardContent, Card, Stack, Button, Select } from "@mui/material";
...
const postings = [
    ...
    },
    {
        ...
        description: "We are looking for a front desk associate to join our coding bootcamp and help provide an excellent experience for our students and visitors. The ideal candidate will be friendly, organized, and knowledgeable about technology and coding practices. \n \n Key Responsibilities: \n • Greet and assist students and visitors upon arrival \n • Manage incoming calls and customer inquiries \n • Schedule and confirm appointments and class reservations \n • Process payments and handle cash transactions accurately \n • Maintain a clean and organized front desk area \n • Provide information about coding classes and services offered \n • Assist with the preparation and setup of coding classes and events \n • Maintain accurate and up-to-date student information in the database",
        applied: false
    },
    {
    ...
    <CardContent>
        <Stack spacing={5} >
            ...
            <Stack direction="row" spacing={1.5} alignItems={"center"}>
                ...
                <Typography variant="subtitle1" color="secondary">{city}, {state}</Typography>
            </Stack>
            ...
            { !applied ?  (<Button color="secondary" sx={{width: "20%"}} variant="contained" onClick={() => setApplied(true)}>Apply</Button>) :  (<Button color="secondary" sx={{width: "20%"}} variant="outlined">Applied</Button>)}
...
const Jobboard = () => {
    const [applied,setApplied] = useState(false)
    return (
        <Container sx={{ mt: 5 }} maxWidth="100vw" style={{ background: '#f0ebf7' }} >
            <Navbar isAuth={true} userType={"Job Seeker"} />
            <Stack sx={{ pt: 10 }} spacing={3}  justifyContent="flex-start" >
                {postings.map((el) =>
                <JobCard title={el.title} city={el.city} state={el.state} exchange={el.exchange} description={el.description} />
                )}
            </Stack>
